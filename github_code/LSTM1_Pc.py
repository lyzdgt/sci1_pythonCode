...
model .add(LSTM (units =256, activation = 'relu', recurrent_activation ='hard_sigmoid'
                 ,return_sequences=True
                 ,input_shape = (n_in,1)))
model .add(LSTM (units =n_out, activation = 'linear', recurrent_activation ='hard_sigmoid'
                 ,input_shape = (n_in,1)))
adam = keras.optimizers.Adam(lr = 0.0005, beta_1=0.95, beta_2=0.999,epsilon=1e-08)
model.compile(loss='mae', optimizer=adam)
model.fit(train_X.reshape(train_X.shape[0],train_X.shape[1],1),train_Y.reshape(train_Y.shape[0],1),epochs=400,shuffle=True,batch_size=7)
pre1=[]
