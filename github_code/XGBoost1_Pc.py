...
reg=XGBR(n_estimators=40,learning_rate=0.09,max_depth=8,eta=0.1).fit(train_X,train_Y)
pre0=reg.predict(train_X)
pre1=[]
txt1=test_X[0]
for i in range(len(test_X)):
    txt1=txt1.reshape(i+1,n_in,1)
    list_xgt=reg.predict(txt1[i].reshape(1,n_in))
    pre1.append(list_xgt)
    list_xgtt=np.concatenate([txt1[i,1:n_in].reshape(n_in-1),list_xgt])
