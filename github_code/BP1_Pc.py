...
model = Sequential()
model.add(Dense(units = 128,  activation='tanh', input_shape =(n_in,)))
...
model.fit(train_X.reshape(train_X.shape[0],train_X.shape[1]),train_Y,epochs=50,shuffle=True,batch_size=1)
...
for i in range(len(test_X)):
    ...
    list_xgt=model.predict(txt1[i].reshape(1,n_in))
...
pre0=model.predict(train_X.reshape(train_X.shape[0],train_X.shape[1]))
